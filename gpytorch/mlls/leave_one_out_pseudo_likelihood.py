...
class LeaveOneActionOutPseudoLikelihood(MarginalLogLikelihood):
    r"""
    The leave-one-(action)-out cross-validation (LOO-CV) likelihood of a computation-aware Gaussian process with a
    Gaussian likelihood.

    This offers an alternative to the exact marginal log likelihood where we
    instead maximize the sum of the leave one *action* out log probabilities :math:`\log p(s_j^{\\top} y | X, S_{-j}^{\\top} y, \\theta)`.
