...
from ..utils import broadcasting, pivoted_cholesky, kmeans
...
class AddedDiagLazyTensor(SumLazyTensor):
    ...
    def _preconditioner(self):
        ...
        elif settings.kmeans.on():
            return self._kmeans_preconditioner()
    ...
    def _kmeans_preconditioner(self):
        if self._q_cache is None:
            ...
            _, z = kmeans.KMeans(x, K = max_iter, Niter = 20, verbose=False)
    ...
    def _nyssvd_preconditioner(self):
        if self._q_cache is None:
            ...
            if settings.record_maxeig:
                settings.record_maxeig.lst_maxeig.append( (S.min() ** 2).item() )
